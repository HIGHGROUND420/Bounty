...
const AIRTABLE_CONFIG = {
  apiKey: import.meta.env.VITE_AIRTABLE_API_KEY || "YOUR_AIRTABLE_PAT_KEY",
  baseId: import.meta.env.VITE_AIRTABLE_BASE_ID || "YOUR_BASE_ID",
...
const callGemini = async (messages, contextData) => {
  ...
  if (!apiKey) {
    return "AI-чат работает в демо-режиме. Пожалуйста, настройте API ключ Gemini для полноценной работы! 🤖";
  }
...
const Notification = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-4 left-4 right-4 z-50 p-4 rounded-xl shadow-xl border backdrop-blur-md transform transition-all duration-300 animate-slide-down flex items-center gap-3
...
const Button = ({
  ...
  const baseStyle =
    "w-full py-3.5 rounded-2xl font-semibold text-sm transition-all duration-200 transform active:scale-95 flex items-center justify-center gap-2 shadow-sm";
  const variants = {
    primary:
      "bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-70 disabled:cursor-not-allowed shadow-slate-300/50",
    secondary:
...
const AIChat = ({ isOpen, onClose, contextData }) => {
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Привет! 👋 Я ваш AI-гид по Таиланду. Помочь выбрать экскурсию или подсказать что-то по обмену валют?",
      sender: "ai",
    },
  ]);
...
const MOCK_EXCURSIONS = [
  {
    ...
    duration: "Весь день",
    image:
      "https://images.unsplash.com/photo-1537956965359-3578e76a951d?auto=format&fit=crop&q=80&w=800",
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1589394815804-964ed0be2eb5?auto=format&fit=crop&q=80&w=800",
  ...
  },
  {
    ...
    duration: "3 часа",
    image:
      "https://images.unsplash.com/photo-1585970480901-90d6b92d8749?auto=format&fit=crop&q=80&w=800",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const isConfigured =
      ...
      AIRTABLE_CONFIG.apiKey !== "YOUR_AIRTABLE_PAT_KEY";
    ...
    const fetchExcursions = async () => {
      try {
        const response = await fetch(
          `https://api.airtable.com/v0/${AIRTABLE_CONFIG.baseId}/${AIRTABLE_CONFIG.excursionsTable}`,
          {
            headers: {
        ...
        const loadedExcursions = data.records.map((record) => {
          ...
          let imageUrl =
            "https://images.unsplash.com/photo-1537956965359-3578e76a951d?auto=format&fit=crop&q=80&w=800"; // Fallback
  ...
  const submitToAirtable = async (table, data) => {
    ...
    try {
      const response = await fetch(
        `https://api.airtable.com/v0/${AIRTABLE_CONFIG.baseId}/${table}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${AIRTABLE_CONFIG.apiKey}`,
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-10">
      ...
      <div className="px-4 py-4 sticky top-[120px] z-20 bg-slate-50/95 backdrop-blur-sm">
        <div className="bg-white p-1.5 rounded-2xl flex relative shadow-sm border border-slate-100">
          <div
            className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-slate-900 rounded-xl shadow-md transition-all duration-300 ease-out ${activeTab === "exchange" ? "translate-x-[calc(100%+6px)]" : "translate-x-0"}`}
          />
          <button
            onClick={() => setActiveTab("excursion")}
            className={`flex-1 relative z-10 flex items-center justify-center gap-2 py-2.5 text-sm font-medium transition-colors duration-200 ${activeTab === "excursion" ? "text-white" : "text-slate-500"}`}
          >
            <Palmtree size={16} />
            <span>Экскурсии</span>
          </button>
          <button
            ...
            className={`flex-1 relative z-10 flex items-center justify-center gap-2 py-2.5 text-sm font-medium transition-colors duration-200 ${activeTab === "exchange" ? "text-white" : "text-slate-500"}`}
          >
            <RefreshCcw size={16} />
            <span>Обмен</span>
          </button>
      ...
      <main className="px-4 overflow-hidden max-w-md mx-auto">
        {/* --- ЭКСКУРСИИ --- */}
        <div
          className={`transition-all duration-500 ease-in-out ${activeTab === "excursion" ? "opacity-100 translate-x-0" : "opacity-0 hidden -translate-x-10"}`}
        >
          {/* VIEW: КАТАЛОГ */}
          {excursionView === "catalog" && (
            <div className="space-y-4 animate-slide-up pb-20">
              ...
              >
                <div>
                  <h3 className="font-bold flex items-center gap-2">
                    <Sparkles size={16} /> Подбор тура с AI
                  </h3>
                  <p className="text-xs text-indigo-100 mt-1">
                    Не знаете что выбрать? Спросите!
                  </p>
                </div>
              ...
              {excursions.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-3xl p-3 shadow-sm border border-slate-100 hover:shadow-md transition-shadow"
                >
                  <div className="relative h-40 rounded-2xl overflow-hidden mb-3">
                    ...
                    />
                    <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-slate-900 shadow-sm">
                      {item.price}
                    </div>
                  </div>
                  <div className="px-1">
                    ...
                    <Button
                      onClick={() => handleExcursionSelect(item)}
                      variant="secondary"
                      className="bg-slate-50 border-0 text-slate-900 hover:bg-slate-100"
                    >
                      Выбрать
                    </Button>
                  </div>
          ...
          {excursionView === "booking" && (
            <div className="animate-slide-up">
              <button
                onClick={() => setExcursionView("catalog")}
                className="mb-4 flex items-center gap-1 text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors"
              >
              ...
              <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 mb-4">
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    submitToAirtable(AIRTABLE_CONFIG.bookingsTable, {
                      ...excursionForm,
                ...
                >
                  ...
                  <Input
                    icon={MapPin}
                    type="text"
                    placeholder="Отель проживания"
                    required
                    value={excursionForm.hotel}
                    onChange={(e) =>
                      setExcursionForm({
                        ...excursionForm,
        ...
        {/* --- ОБМЕН --- */}
        <div
          className={`transition-all duration-500 ease-in-out ${activeTab === "exchange" ? "opacity-100 translate-x-0" : "opacity-0 hidden translate-x-10"}`}
        >
          <form
            onSubmit={(e) => {
              e.preventDefault();
              submitToAirtable(AIRTABLE_CONFIG.exchangeTable, exchangeForm);
            }}
            className="space-y-4 animate-slide-up pb-20"
          >
            <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-4">
              ...
              <Input
                ...
                onChange={(e) =>
                  setExchangeForm({ ...exchangeForm, amount: e.target.value })
                }
              />
              <Select
                icon={RefreshCcw}
                value={exchangeForm.pair}
                onChange={(e) =>
                  setExchangeForm({ ...exchangeForm, pair: e.target.value })
                }
                placeholder="Валютная пара"
                options={[
                  { value: "RUB_USDT", label: "RUB → USDT" },
                  { value: "USDT_RUB", label: "USDT → RUB" },
                  { value: "USD_THB", label: "USD → THB" },
                  { value: "THB_USD", label: "THB → USD" },
                ]}
              />
                ...
                <div className="flex gap-2">
                  {["cash", "card"].map((m) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setExchangeForm({ ...exchangeForm, payMethod: m })
                      }
                      className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${exchangeForm.payMethod === m ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:bg-slate-200"}`}
                    >
                ...
                <div className="flex gap-2">
                  {["cash", "card"].map((m) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setExchangeForm({ ...exchangeForm, receiveMethod: m })
                      }
                      className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${exchangeForm.receiveMethod === m ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:bg-slate-200"}`}
                    >
                      {m === "cash" ? "Наличные" : "Карта"}
                    </button>
                  ))}
                ...
                {exchangeForm.receiveMethod === "card" && (
                  <div className="animate-fade-in-up">
                    <Select
                      icon={Building2}
                      placeholder="Банк"
                      value={exchangeForm.receiverBank}
                      onChange={(e) =>
                        setExchangeForm({
                          ...exchangeForm,
